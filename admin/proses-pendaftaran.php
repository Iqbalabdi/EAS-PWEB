...
if (isset($_POST['daftar'])) {
    ...
    if(isset($_POST['ubah_foto'])){
        ...
        if(move_uploaded_file($tmp, $path)){
            ...
            if ($query) {
                header('Location: siswa-management.php?status=sukses');
            } else {
                header('Location: siswa-management.php?status=gagal');
            }
        } else{
        ...
        $sql = "Insert into siswa (nama, alamat, jenis_kelamin, agama, sekolah_asal) values ('$nama', '$alamat', '$jk', '$agama', '$sekolah')";
        $query = mysqli_query($db, $sql);
